  ...
  var Monitor = Backbone.Model.extend({
    ...
    */
    onChange: function() {
      ...
      // Don't write if probe isn't connected
      if (!t.probe) {
        log.warn('probeSet', 'Attempting to set a disconnected probe');
        return;
      }
      ...
      for (var attrName in probeAttrs) {
        ...
        if (isWritable && t.attributes[attrName] !== t._previousAttributes[attrName]) {
