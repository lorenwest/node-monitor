...
*   
* Output:
*   mouduleMonitors - A dictionary of module monitors.
...
*/
nodeMonitor.exitReport = function() {
  ...
  // No report if no monitors were fired
...
* gracefulExit
******************************************************************************
* This performs a graceful exit by allowing other SIGINT and SIGTERM hooks
* to process, then exiting causing the exit report to be generated.
*/
nodeMonitor.gracefulExit = function() {
  // Schedule a graceful exit
  process.nextTick(function(){process.exit(0);});
};

// Hook process events
process.on('exit', nodeMonitor.exitReport);
process.on('SIGINT', nodeMonitor.gracefulExit);
process.on('SIGTERM', nodeMonitor.gracefulExit);
