// MonitorUI.js (c) 2012 Loren West and other contributors
...
(function(root){
  ...
  */
  UI.containedModel = function(model, attrName, ctor) {
    ...
    function setAttr(setModel, newValue, setOptions) {
      ...
      // Pass through if setting to the correct type or removing
      if (newValue === undefined || newValue === null || newValue instanceof ctor) {
      ...
      newValue = model._containedModels[attrName] = model.attributes[attrName] = new ctor(newValue, setOptions);

      // Watch for changes to the underlying model or collection
      // (collection add/remove), forwarding changes to this model
      newValue.on('change add remove', UI.onSubModelChange, {
        parent:model,
        attrName: attrName
      });
    }
    ...
    if (subModel instanceof Backbone.Model || subModel instanceof Backbone.Collection) {
      subModel.on('change add remove', UI.onSubModelChange, {
        parent:model,
        attrName: attrName
      });
    }
  ...
  */
  UI.onSubModelChange = function(model) {
    ...
    if (!chgOptions.stopPropagation && !chgOptions.silent && !parent._changing) {

      // Use setTimeout to bundle up all changes into a single change event
      // for each attribute and the parent class.
      (parent._pendingChanges = (parent._pendingChanges || [])).push(attrName);
      window.setTimeout(function(){
        if (parent._pendingChanges && !chgOptions.stopPropagation) {
          parent._pendingChanges.forEach(function(chgAttrName) {
            parent.trigger('change:' + chgAttrName, parent, parent.get(chgAttrName), chgOptions);
          });
          parent.trigger('change', parent, chgOptions);
        }
