  ...
  */
  var PageView = UI.PageView = Backbone.View.extend({
    ...
    render: function() {
      ...
      t.navbar = t.$('.nm-pv-tour');

      // Attach the about dialog
      t.$el.append(aboutTemplate.apply(params));

      // Instantiate the sidebar & page tour
      t.sidebarView = new UI.SidebarView({model: model, el: t.sidebar, pageView:t});
      t.sidebarView.render();
      var tourView = new UI.TourView({model: model, el: t.navbar, pageView:t});
      tourView.render();
