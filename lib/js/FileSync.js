// FileSync.js (c) 2012 Loren West and other contributors
  ...
  */
  var FileSync = SyncProbe.FileSync = Probe.extend({
    ...
    initialize: function(attributes, options){
      ...
      t.fullPath = Path.join(t.dirpath, t.get('modelId') + '.json');
      var onFileWatch = function(error, content) {
        ...
        try {
          ...
          return console.error('File format error - invalid JSON: ' + t.fullPath + ' Content: ' + content);
        }
      ...
      t.fileWatcher = FileProbe.watchLoad(t.fullPath, watcherOpts, onFileWatch);
    },
    ...
    create_control: function(args, callback) {
      ...
      var fullPath = Path.join(t.dirpath, model.id + '.json');
      if (fullPath.indexOf(t.dirpath) !== 0) {
        return callback({msg:'ID cannot represent a relative path'});
      }
    ...
    read_control: function(args, callback) {
      ...
      // Build the full path to the file
      var fullPath = Path.join(t.dirpath, id + '.json');
      if (fullPath.indexOf(t.dirpath) !== 0) {
        return callback({msg:'ID cannot represent a relative path'});
      }
