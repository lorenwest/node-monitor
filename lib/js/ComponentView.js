// ComponentView.js (c) 2012 Loren West and other contributors
  ...
  */
  var ComponentView = UI.ComponentView = Backbone.View.extend({
    ...
    render: function() {
      ...
      // Monitor changes to the monitor definition
      ...
      var numToConnect = t.connectMonitor(true, function(error) {
    ...
    */
    close: function() {
      var t = this;
      var components = UI.pageView.model.get('components');
      components.remove(t.model.get('id'));
    },
    ...
    */
    connectMonitor: function(connect, callback) {
      ...
      // If no need to connect, callback now
      if (!needsConnecting) {
      ...
      if (connect) {
        ...
        monitor.set(
          ...
          {silent: true, stopPropagation: true}
        );

        // Don't mark initParams as having been changed in silent
        // (since they've been restored to their original value)
        delete monitor._silent.initParams;
      }
    ...
    openSettings: function(e) {
      ...
      // Replace any inner custom settings view
      if (customSettings) {
        customSettings.remove();
      }
      if (t.viewClass['SettingsView']) {
        customSettings = new t.viewClass['SettingsView']({
        ...
        settingsView.$('.nm-cs-view-settings').append(customSettings.el);
        customSettings.render();
      }

      // Set the component model into the component view
      settingsView.setModel(t.model);
    ...
    dragStart: function(e) {
      var t = this,
          ...
          isUnlocked = $('.nm-pv').hasClass('unlocked'),
          persist = true;  // Persist changes to the model?

      // OK to drag if unlocked or we're dragging from the title bar
      if (!isUnlocked && !inTitle) {
        return;
      }
    ...
    resizeStart: function(e) {
      ...
      function drop(e) {
        ...
        // Set new height/width into the model if it's changed and we're unlocked
