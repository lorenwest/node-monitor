  ...
  */
  var Component = UI.Component = Backbone.Model.extend({
    ...
    initialize: function(params, options) {
      ...
      // Don't propagate probe-only changes to the component (and page)
      t.get('monitor').on('change', t.preventProbePropagation);
    },
    ...
    */
    ...
    // This method prevents probe-only changes from bubbling up to the
    // component (and page).
    preventProbePropagation: function(monitor, options) {
      var monitorFields = Monitor.prototype.defaults,
          allChanges = (options && options.changes ? options.changes :  {}),
          monitorChanges = _.intersection(_.keys(monitorFields), _.keys(allChanges));

      if (_.isEmpty(monitorChanges)) {
