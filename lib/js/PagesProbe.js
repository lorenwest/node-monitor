...
(function(root){
  ...
  var ROOT_PATH = Path.resolve(__dirname + '../../../site_db/Page');
  ...
  var PagesProbe = Monitor.PagesProbe = TreeProbe.extend({
    ...
    */
    release: function() {
      ...
      for (var pageId in t.pageWatchers) {
        t.pageWatchers[pageId].close();
      }
      t.pageWatchers = {};
    },
    ...
    */
    sync: function(callback) {
      ...
      FS.readdir(t.dirPath, function(error, fileNames) {
        ...
        // Process errors
        if (error) {
          console.error('PagesProbe readdir error', error);
        ...
        t.statFiles(fileNames, function(error, fileStats) {
          ...
          // Process errors
          if (error) {
            console.error('PagesProbe statFiles error', error);
            return callback(error);
          }
    ...
    readPages: function(pages, callback) {
      ...
      pages.forEach(function(page, index) {
        ...
        FS.readFile(fullPath, 'utf8', function(error, file) {
          ...
          // Add a page watcher if not already watching
          if (!t.pageWatchers[page.id]) {
            t.pageWatchers[page.id] = new FS.watch(fullPath, {persistent:false}, function(type){
              t.sync()
            });
          }
