...
(function(root){
  ...
  var ROOT_PATH = Path.resolve(process.cwd() + '/site_db/Page');
    ...
    */
    initialize: function(attributes, options){
      ...
      if (appName && UI.Server.currentServer.apps[appName]) {
        ...
        parts.splice(0,2);
        t.dirPath = Path.join(appDef.moduleDir + '/site_db/Page', parts.join('/'));
      }
      else {
      ...
      FS.readdir(t.dirPath, function(error, fileNames) {
        ...
        t.statFiles(fileNames, function(error, fileStats) {
          ...
          t.readPages(pages, function(error) {
            ...
            if (t.dirPath === ROOT_PATH) {
              dirs.push({id:'app', label:'App Pages', description:'Application pages'});
            }
