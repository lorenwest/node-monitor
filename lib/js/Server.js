  ...
  var Server = UI.Server = Backbone.Model.extend({
    ...
    */
    loadApp: function(moduleDir) {
      ...
      var moduleDbPath = moduleDbDir + shortName;
      var appHomePage = shortName + '.json';
      var appHomePagePath = moduleDbDir + appHomePage;
      ...
      // Make a symbolic link to the app home page if it exists
      try {
        // Will throw ENOENT if not there
        FS.statSync(appHomePagePath);
        // Will throw EEXIST if already linked
        FS.symlinkSync(appHomePagePath, appDbDir + appHomePage, 'file');
      } catch (e) {
