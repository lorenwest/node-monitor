  ...
  */
  var Server = UI.Server = Backbone.Model.extend({
    ...
    initialize: function(params, options) {
      ...
      // Normalize the site db path

      siteDbPath = Path.join(siteDbPath.indexOf('.') === 0 '',)

      // Initialize probes
      siteDbPath = Path.join(siteDbPath.indexOf('.') === 0 '',)
      SyncProbe.Config.defaultProbe = 'FileSync';
      SyncProbe.FileSync.setRootPath(process.cwd() + '/site_db');
    ...
    */
    _initializeSite: function(callback) {
      ...
      t.site.save({}, {liveSync: true}, function(error) {
        ...
        // Load a different index.json template for monitor apps
        v
