  ...
  */
  var Server = UI.Server = Backbone.Model.extend({
    ...
    initialize: function(params, options) {
      ...
      // Create a static server to the monitor-min distribution
      var minDir = require.resolve('monitor-min').replace('lib/index.js', 'dist');
      t.monitorMinDist = Connect['static'](minDir);
    ...
    */
    siteRoute: function(request, response, next) {
      ...
      if (urlParts[0] === 'static') {
        ...
        // Forward to the monitor-min distribution
        if (request.url.indexOf('monitor-min-all.js') > 0) {
          request.url = '/monitor-min-all.js';
          return t.monitorMinDist(request, response, next);
        }
    ...
    */
    loadApp: function(moduleDir, packageJson) {
      ...
      try {
        FS.readdirSync(moduleDir + '/lib/view').forEach(function(filename) {
      ...
      // Add extensions
      try {
        FS.readdirSync(moduleDir + '/lib/ext').forEach(function(filename) {
          var ext = Path.extname(filename).toLowerCase();
          var base = Path.basename(filename, ext);
          if (ext === '.js') {
      ...
      try {
        FS.readdirSync(moduleDir + '/lib/css').forEach(function(filename) {
          var ext = Path.extname(filename).toLowerCase();
          var base = Path.basename(filename, ext);
          if (ext === '.css') {
            css.push(filename);
      ...
      try {
        FS.readdirSync(moduleDir + '/lib/template').forEach(function(filename) {
          var ext = Path.extname(filename).toLowerCase();
          var base = Path.basename(filename, ext);
          if (ext === '.html') {
      ...
      // Gather images
      try {
        FS.readdirSync(moduleDir + '/lib/image').forEach(function(filename) {
          var ext = Path.extname(filename).toLowerCase();
          var base = Path.basename(filename, ext);
