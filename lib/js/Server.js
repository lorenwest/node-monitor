  ...
  var Server = UI.Server = Backbone.Model.extend({
    ...
    _initializeSite: function(callback) {
      ...
      t.site.save({}, {liveSync: true}, function(error) {
        ...
        pages.forEach(function(page) {
          ...
          var model = new Page(JSON.parse(tmpl.get('text')));
          model.save(whenDone);
        });
      });
    },
    ...
    */
    loadApp: function(moduleDir) {
      ...
      var dbDir = Path.normalize(__dirname + '/../../site_db/Page/app/');
      var moduleDbDir = moduleDir + '/page_db/';
      try {
        FS.mkdirSync(dbDir);
      } catch (e) {
        if (e.code !== 'EEXIST') {
      ...
      // Make a symbolic link to the app home page
      try {
        var appHome = appName + '.json';
        FS.symlinkSync(moduleDbDir + appHome, dbDir + appHome, 'file');
      } catch (e) {
        if (e.code !== 'EEXIST') {
          console.error('Cannot link page home for app: ' + appName, e);
        }
      }

      // Make a symbolic link to the app page db
      try {
        FS.symlinkSync(moduleDbDir + appName, dbDir + appName, 'dir');
      } catch (e) {
        if (e.code !== 'EEXIST') {
          console.error('Cannot link page DB for app: ' + appName, e);
        }
      }
