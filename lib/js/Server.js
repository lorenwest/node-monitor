...
(function(root){
  ...
  SyncProbe.FileSync.setRootPath(Path.normalize(__dirname + '/../../site_db'));
    ...
    */
    loadApps: function() {
      ...
      var loadNodeModulesDir = function(dir) {
        ...
        // Test the application directory above node_modules
        var dirs = ['..'];
        ...
        dirs.forEach(function(moduleName) {
          ...
          var isMonitorUIApp = pkg.dependencies && _.find(_.keys(pkg.dependencies), function(keyword){ return keyword === 'monitor'; });
          if (!isMonitorUIApp) {
            return;
          }

          // Load the app
          t.loadApp(moduleDir, pkg);
